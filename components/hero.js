...
function Hero({homepage}) {
  ...
  return (
    <div className="absolute inset-0 bg-gray-400 w-full h-screen">
      <Image loader={myLoader} src={imageUrl} alt="nature unplash" layout="fill" className="w-full h-full object-cover" unoptimized/>
      <div className="h-1/2 w-full opacity-1 bg-gradient-to-t from-white z-10 absolute bottom-0 left-0"></div>
    </div>
  )
}
